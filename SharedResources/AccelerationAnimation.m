...
+ (id)animationWithKeyPath:(NSString *)keyPath
	...
	evaluationObject:(CubicBezierInterpolator *)evaluationObject
...
- (void)calculateKeyFramesWithEvaluationObject:(CubicBezierInterpolator *)evaluationObject
	startValue:(double)startValue
	endValue:(double)endValue
	interstitialSteps:(NSUInteger)steps {
	...
	for (i = 0; i < count; i++) {
		double value = startValue + [evaluationObject interpolation:progress] * (endValue - startValue);
